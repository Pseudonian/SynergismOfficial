...
function displayruneinformation(i, updatelevelup) {
  ...
  if (player.upgrades[71] == 1 && i == 2) {
    s = player.runelevels[1]
  }
  if (player.upgrades[71] == 1 && i == 3) {
    s = player.runelevels[2]
  }
  if (player.upgrades[71] == 1 && i == 4) {
    s = player.runelevels[3]
  }
  if (player.upgrades[71] == 1 && i == 5) {
    s = player.runelevels[4]
  }


  if (i == 1) {
    if (updatelevelup) {
      document.getElementById("runeshowlevelup").textContent = "+1 Accelerator, +0.5% Accelerators per level. +1 Accelerator Boost every 10 levels!"
    }
  ...
  if (i == 2) {
    if (updatelevelup) {
      document.getElementById("runeshowlevelup").textContent = "~(floor(Level/10)) Multipliers every 10 levels, +0.5% Multipliers per level. Tax growth is delayed more for each level!"
    }
  ...
  if (i == 3) {
    if (updatelevelup) {
      document.getElementById("runeshowlevelup").textContent = "~(1 + Level^2 * 2^Level / 256)x Crystal Production. +1 free level for each Crystal upgrade per 10 levels!"
    }
  ...
  if (i == 4) {
    if (updatelevelup) {
      document.getElementById("runeshowlevelup").textContent = "+0.25% building cost growth delay per level, +0.125% offering recycle chance per level [MAX: 25%], 2^((200 - Level)/550) Tax growth multiplier AFTER level 200"
    }
  ...
  if (i == 5) {
    if (updatelevelup) {
      document.getElementById("runeshowlevelup").textContent = "~(2^(level/300) * (1 + level/150))x Obtainium, 1 + Level^2/1440 Ant Hatch Speed, +0.4 * level seconds of offering timer extension."
    }
...
function resetofferings(i) {
  ...
  if (i >= 2) {
    ...
    if (player.achievements[44] > 0.5) {
      b += (15 * Math.min(1, player.transcendcounter / 1800))
    }
  ...
  if (i >= 1) {
    ...
    if (player.achievements[37] > 0.5) {
      c += (15 * Math.min(1, player.prestigecounter / 1800))
    }
  ...
  if (player.upgrades[75] > 0.5 && player.maxobtainium !== undefined) {
...
function redeemshards(runeIndexPlusOne, auto, autoMult) {
  ...
  if (auto) {
    amount = Math.min(player.runeshards, Math.pow(2, 1 + player.shopUpgrades.offeringAutoLevel) * autoMult)
  }
  // recycleMult accounted for all recycle chance, but inversed so it's a multiplier instead
  let recycleMult = 1 / (1 - 0.05 * player.achievements[80] - 0.05 * player.achievements[87] - 0.05 * player.achievements[94] - 0.05 * player.achievements[101] - 0.05 * player.achievements[108] - 0.05 * player.achievements[115] - 0.075 * player.achievements[122] - 0.075 * player.achievements[129] - 0.05 * player.upgrades[61] - Math.min(0.25, rune4level / 800));

  if (runeIndex == 0) {
    res1mult = (1 + player.researches[78] / 250);
  }
  if (runeIndex == 1) {
    res1mult = (1 + player.researches[80] / 250);
  }
  if (runeIndex == 2) {
    res1mult = (1 + player.researches[79] / 250);
  }
  if (runeIndex == 3) {
    res1mult = (1 + player.researches[77] / 250);
  }
  if (runeIndex == 4) {
    res1mult = 1;
    SIMult1 = (1 + player.researches[83] / 50);
  }
  if (player.runeshards >= 1) {
    ...
    if (player.runelevels[runeIndex] < (500 + increaseMaxLevel[runeIndex] + increaseMaxLevel2[runeIndex]) && (runeIndex == 0 || (runeIndex == 1 && player.achievements[38] == 1) || (runeIndex == 2 && player.achievements[44] == 1) || (runeIndex == 3 && player.achievements[102] == 1) || (runeIndex == 4 && player.researches[82] == 1))) {
      ...
      if (player.upgrades[71] > 0.5) {
        s += player.runelevels[runeIndex]
      }
      ...
      if (player.upgrades[66] > 0.5) {
        ...
        if (player.achievements[38] > 0.5) {
          player.runeexp[1] += 3 * amount * recycleMult
        }
        if (player.achievements[44] > 0.5) {
          player.runeexp[2] += 3 * amount * recycleMult
        }
        if (player.achievements[102] > 0.5) {
          player.runeexp[3] += 3 * amount * recycleMult
        }
        if (player.researches[82] > 0.5) {
          player.runeexp[4] += 3 * amount * recycleMult
        }
      }
      ...
      for (let runeToUpdate = 0; runeToUpdate < 5; ++runeToUpdate) {
        ...
        let r = 1;
        if (player.challengecompletions.six > 0.5 && runeToUpdate == 3) {
          r -= 0.02 * player.challengecompletions.six
        }
        if (player.challengecompletions.seven > 0.5 && (runeToUpdate == 0 || runeToUpdate == 1)) {
          r -= 0.02 * player.challengecompletions.seven
        }
