...
const antUpgradeTexts = [null,
  function() {
    return "ALL Ants work at " + format(Math.pow(1.11 + 1 / 1000 * player.researches[101], player.antUpgrades[1] + bonusant1), 2) + "x speed."
  },
  function() {
    return "Crumb --> Coin exponent is ^" + format(100000 + 900000 * (1 - Math.pow(2, -(player.antUpgrades[2] + bonusant2) / 125)))
  },
  function() {
    return "Tax growth is multiplied by " + format(0.01 + Math.pow(0.98, player.antUpgrades[3] + bonusant3 + 0.497), 4)
  },
  function() {
    return "Accelerator Boosts +" + format(2 * player.antUpgrades[4] + 2 * bonusant4) + "%"
  },
  function() {
    return "Multipliers +" + format(4 * player.antUpgrades[5] + 4 * bonusant5) + "%"
  },
  function() {
    return "Offerings x" + format(calculateSigmoid(5, (player.antUpgrades[6] + bonusant6), 125), 4)
  },
  function() {
    return "Building Costs scale " + format(3 * player.antUpgrades[7] + 3 * bonusant7) + "% slower!"
  },
  function() {
    return "Rune EXP is multiplied by " + format(Math.pow(1.01, player.antUpgrades[8] + bonusant8), 2) + "!"
  },
  function() {
    return "Each rune has +" + format(player.antUpgrades[9] + bonusant9) + " effective levels."
  },
  function() {
    return "Obtainium x" + format(calculateSigmoid(10, (player.antUpgrades[10] + bonusant10), 125), 4)
  },
  function() {
    return "Sacrificing is " + format(1 + 2 * (1 - Math.pow(2, -(player.antUpgrades[11] + bonusant11) / 125)), 4) + "x as effective"
  },
  function() {
    return "Unlocks the talisman!"
  }
]
...
function updateAntDescription(i) {
  ...
  switch (i) {
    case 1:
      priceType = "Particles";
      tier = "first";
      me.textContent = "Generates " + format(antOneProduce, 4) + " Crumbs/sec";
      break;
    case 2:
      tier = "second";
      me.textContent = "Generates " + format(antTwoProduce, 4) + " Workers/sec";
      break;
    case 3:
      tier = "third";
      me.textContent = "Generates " + format(antThreeProduce, 4) + " Breeders/sec";
      break;
    case 4:
      tier = "fourth";
      me.textContent = "Generates " + format(antFourProduce, 4) + " MetaBreeders/sec";
      break;
    case 5:
      tier = "fifth";
      me.textContent = "Generates " + format(antFiveProduce, 4) + " MegaBreeders/sec";
      break;
    case 6:
      tier = "sixth";
      me.textContent = "Generates " + format(antSixProduce, 4) + " Queens/sec";
      break;
    case 7:
      tier = "seventh";
      me.textContent = "Generates " + format(antSevenProduce, 4) + " Royals/sec";
      break;
    case 8:
      tier = "eighth";
      me.textContent = "Generates " + format(antEightProduce, 4) + " ALMIGHTIES/sec";
      break;
  }
...
function buyAntUpgrade(i, auto) {
  if (player.antPoints.greaterThanOrEqualTo(Decimal.pow(10, antUpgradeCostIncreases[i] * player.antUpgrades[i]).times(antUpgradeBaseCost[i]))) {
    ...
    if (!auto) {
      antUpgradeDescription(i)
    }
    if (player.antUpgrades[12] == 1 && i == 12) {
      revealStuff()
    }
  }
}
...
function sacrificeAnts() {
  ...
  if (player.antPoints.greaterThanOrEqualTo("1e40")) {
    ...
    if (p) {
      ...
      if (player.challengecompletions.nine > 0.5) {
        if (antELO > 500) {
          player.talismanShards += Math.floor((timeMultiplier * mult * Math.pow(1 / 4 * (antELO - 500), 2)))
        }
        if (antELO > 750) {
          player.commonFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 9 * (antELO - 750), 1.83)))
        }
        if (antELO > 1000) {
          player.uncommonFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 16 * (antELO - 1000), 1.66)))
        }
        if (antELO > 1500) {
          player.rareFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 25 * (antELO - 1500), 1.5)))
        }
        if (antELO > 2000) {
          player.epicFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 36 * (antELO - 2000), 1.33)))
        }
        if (antELO > 3000) {
          player.legendaryFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 49 * (antELO - 3000), 1.16)))
        }
        if (antELO > 5000) {
          player.mythicalFragments += Math.floor((timeMultiplier * mult * Math.pow(1 / 64 * (antELO - 5000), 1)))
        }
      }
      resetAnts();
